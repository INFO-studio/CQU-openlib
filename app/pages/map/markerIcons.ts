...
/**
 * Lucide 0.561 的原始 icon nodes。高德 Marker 接收 DOM/HTML 而不是 React
 * 节点，因此保留这份静态 markup，避免为了 15 个小图标引入 react-dom/server。
 */
...
} as const satisfies Readonly<Record<BuildingCategory, string>>;
...
export const MapCategoryIcon = ({
  ...
  createElement('svg', {
    ...
    className,
    'aria-hidden': true,
    dangerouslySetInnerHTML: { __html: CATEGORY_ICON_NODES[category] },
  });

export const markerCategoryIconMarkup = (category: BuildingCategory) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="position:absolute;top:6.5px;left:11px;color:var(--c-icon-strong)">${CATEGORY_ICON_NODES[category]}</svg>`;
