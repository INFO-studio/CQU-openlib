/**
 * a-d：沙坪坝 A、B、C 校园与科学城虎溪校园；e：两江校区。
 * A、虎溪点位经 CQUMAPS 原作者授权整合；其余点位据重庆大学官方校园图与公开地图数据整理。
 */
export type CampusId = 'a' | 'b' | 'c' | 'd' | 'e';

export type BuildingCategory =
  | 'teaching'
  | 'dormitory'
  | 'canteen'
  | 'library'
  | 'sports'
  | 'admin'
  | 'gate'
  | 'hospital'
  | 'theater'
  | 'busstation'
  | 'transit'
  | 'landmark'
  | 'college'
  | 'food'
  | 'express';

/**
 * 高德 GCJ-02 坐标，依次为经度、纬度。底图用的就是这套，其他地图厂商在
 * `navigation.ts` 里现算——转换只发生在外链上，落点不受累积误差影响。
 */
export type Gcj02Coordinate = readonly [longitude: number, latitude: number];

export interface Campus {
  readonly id: CampusId;
  readonly campusName: string;
  readonly siteName: string;
  readonly center: Gcj02Coordinate;
  /** 初始缩放级别；数值越大，视野越近。 */
  readonly defaultZoom: number;
}

export interface Building {
  readonly id: string;
  readonly name: string;
  readonly category: BuildingCategory;
  readonly campusId: CampusId;
  readonly coord: Gcj02Coordinate;
  readonly desc: string;
}

export interface BuildingCategoryDefinition {
  readonly id: BuildingCategory;
  readonly label: string;
}

export const BUILDING_CATEGORIES = [
  {
    id: 'teaching',
    label: '教学楼',
  },
  {
    id: 'dormitory',
    label: '学生宿舍',
  },
  {
    id: 'canteen',
    label: '食堂',
  },
  {
    id: 'library',
    label: '图书馆',
  },
  {
    id: 'sports',
    label: '运动场馆',
  },
  {
    id: 'admin',
    label: '行政楼',
  },
  {
    id: 'gate',
    label: '校门',
  },
  {
    id: 'hospital',
    label: '校医院',
  },
  {
    id: 'theater',
    label: '剧场',
  },
  {
    id: 'busstation',
    label: '校车站',
  },
  {
    id: 'transit',
    label: '公共交通',
  },
  {
    id: 'landmark',
    label: '地标建筑',
  },
  {
    id: 'college',
    label: '学院楼',
  },
  {
    id: 'food',
    label: '附近美食',
  },
  {
    id: 'express',
    label: '快递点',
  },
] as const satisfies readonly BuildingCategoryDefinition[];

export const CAMPUSES = [
  {
    id: 'd',
    campusName: '科学城校区',
    siteName: '虎溪校园',
    center: [106.2982, 29.593145],
    defaultZoom: 16,
  },
  {
    id: 'a',
    campusName: '沙坪坝校区',
    siteName: 'A校园',
    center: [106.469183, 29.564402],
    defaultZoom: 16,
  },
  {
    id: 'b',
    campusName: '沙坪坝校区',
    siteName: 'B校园',
    center: [106.458901, 29.567418],
    defaultZoom: 16,
  },
  {
    id: 'c',
    campusName: '沙坪坝校区',
    siteName: 'C校园',
    center: [106.453622, 29.560272],
    defaultZoom: 16,
  },
  {
    id: 'e',
    campusName: '两江校区',
    siteName: '卓越工程师学院',
    center: [106.809205, 29.73923],
    defaultZoom: 17,
  },
] as const satisfies readonly Campus[];

export const CAMPUS_BY_ID = Object.fromEntries(
  CAMPUSES.map((campus) => [campus.id, campus]),
) as Readonly<Record<CampusId, Campus>>;

export const BUILDING_CATEGORY_BY_ID = Object.fromEntries(
  BUILDING_CATEGORIES.map((category) => [category.id, category]),
) as unknown as Readonly<Record<BuildingCategory, BuildingCategoryDefinition>>;

export const BUILDINGS = [
  {
    id: 'huxi_teaching_01',
    name: '第一教学楼（D1/一教）',
    category: 'teaching',
    campusId: 'd',
    coord: [106.301748, 29.595204],
    desc: '虎溪校区主教学楼，承担大部分本科生公共课程教学。',
  },
  {
    id: 'huxi_teaching_02',
    name: '综合楼 （DZ）',
    category: 'teaching',
    campusId: 'd',
    coord: [106.29951, 29.596056],
    desc: '本科教学与考研自习常用楼栋。',
  },
  {
    id: 'huxi_teaching_03',
    name: '艺术楼（DY）',
    category: 'teaching',
    campusId: 'd',
    coord: [106.303982, 29.593418],
    desc: '艺术楼，上思政课较多',
  },
  {
    id: 'huxi_teaching_04',
    name: '理科大楼',
    category: 'teaching',
    campusId: 'd',
    coord: [106.29722, 29.59083],
    desc: '理工科基础课程主要授课场所。',
  },
  {
    id: 'huxi_teaching_05',
    name: '信息大楼',
    category: 'teaching',
    campusId: 'd',
    coord: [106.305781, 29.596483],
    desc: '新建教学楼，配备现代化智慧教室。',
  },
  {
    id: 'huxi_teaching_06',
    name: '第一实验楼',
    category: 'teaching',
    campusId: 'd',
    coord: [106.304464, 29.595075],
    desc: '物理、化学、生物等基础理科实验教学中心。',
  },
  {
    id: 'huxi_teaching_07',
    name: '第二实验楼',
    category: 'teaching',
    campusId: 'd',
    coord: [106.300388, 29.591785],
    desc: '物理、化学、生物等基础理科实验教学中心。',
  },
  {
    id: 'huxi_teaching_08',
    name: '学生交叉创新中心',
    category: 'teaching',
    campusId: 'd',
    coord: [106.29319, 29.593647],
    desc: '金工实习课程教室与竞赛实验室',
  },
  {
    id: 'huxi_library_01',
    name: '虎溪图书馆',
    category: 'library',
    campusId: 'd',
    coord: [106.301949, 29.592237],
    desc: '虎溪校区中心图书馆，馆藏丰富，设有多层自习区与研讨室。',
  },
  {
    id: 'huxi_admin_01',
    name: '重庆大学科学中心',
    category: 'admin',
    campusId: 'd',
    coord: [106.289054, 29.597086],
    desc: '虎溪校区机械学院行政办公中心，研究生办公处。',
  },
  {
    id: 'huxi_dorm_01',
    name: '梅园一栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300512, 29.596449],
    desc: '又称梅一，梅园宿舍区，毗邻梅园食堂。',
  },
  {
    id: 'huxi_dorm_02',
    name: '梅园二栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300486, 29.596904],
    desc: '梅园宿舍区，本科生公寓。',
  },
  {
    id: 'huxi_dorm_03',
    name: '梅园三栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300183, 29.597448],
    desc: '梅园宿舍区，本科生公寓。',
  },
  {
    id: 'huxi_dorm_04',
    name: '梅园四栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300495, 29.597915],
    desc: '梅园宿舍区，本科生公寓。',
  },
  {
    id: 'huxi_dorm_05',
    name: '梅园五栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.299769, 29.597828],
    desc: '梅园宿舍区，本科生公寓。',
  },
  {
    id: 'huxi_dorm_06',
    name: '梅园六栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.299487, 29.597393],
    desc: '梅园宿舍区，本科生公寓。',
  },
  {
    id: 'huxi_dorm_07',
    name: '梅园七栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.299562, 29.596983],
    desc: '梅园宿舍区，本科生公寓。',
  },
  {
    id: 'huxi_dorm_08',
    name: '兰园一栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300123, 29.589707],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_09',
    name: '兰园二栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300452, 29.589412],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_10',
    name: '兰园三栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.29897, 29.589574],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_11',
    name: '兰园四栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.299571, 29.58927],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_12',
    name: '兰园五栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.300017, 29.588805],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_13',
    name: '兰园六栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.298264, 29.588981],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_14',
    name: '兰园七栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.298822, 29.588486],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_15',
    name: '兰园八栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.299264, 29.588236],
    desc: '兰园宿舍区，邻近运动场。',
  },
  {
    id: 'huxi_dorm_16',
    name: '竹园一栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.296065, 29.596801],
    desc: '竹园宿舍区，东侧生活组团。',
  },
  {
    id: 'huxi_dorm_17',
    name: '竹园二栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.296939, 29.597266],
    desc: '竹园宿舍区，靠近体育场。',
  },
  {
    id: 'huxi_dorm_18',
    name: '竹园三栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.29688, 29.59768],
    desc: '竹园宿舍区，靠近体育场。',
  },
  {
    id: 'huxi_dorm_19',
    name: '竹园四栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.296391, 29.598117],
    desc: '竹园宿舍区，靠近体育场。',
  },
  {
    id: 'huxi_dorm_20',
    name: '竹园五栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.295551, 29.598147],
    desc: '竹园宿舍区，靠近体育场。',
  },
  {
    id: 'huxi_dorm_21',
    name: '竹园六栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.296222, 29.59749],
    desc: '竹园宿舍区，靠近体育场。',
  },
  {
    id: 'huxi_dorm_22',
    name: '菊园博士生公寓',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.297709, 29.587528],
    desc: '菊园宿舍区，博士生宿舍。',
  },
  {
    id: 'huxi_dorm_23',
    name: '松园一栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.293526, 29.596325],
    desc: '松园宿舍区。',
  },
  {
    id: 'huxi_dorm_24',
    name: '松园二栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.293749, 29.596902],
    desc: '竹园宿舍区，靠近体育场。',
  },
  {
    id: 'huxi_dorm_25',
    name: '松园三栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.293816, 29.597348],
    desc: '松园宿舍区。',
  },
  {
    id: 'huxi_dorm_26',
    name: '松园四栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.294464, 29.597819],
    desc: '松园宿舍区，西南侧生活组团。',
  },
  {
    id: 'huxi_dorm_27',
    name: '松园五栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.294942, 29.596953],
    desc: '松园宿舍区，西南侧生活组团。',
  },
  {
    id: 'huxi_dorm_28',
    name: '松园六栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.295228, 29.596582],
    desc: '松园宿舍区，西南侧生活组团。',
  },
  {
    id: 'huxi_dorm_29',
    name: '松园七栋',
    category: 'dormitory',
    campusId: 'd',
    coord: [106.293323, 29.595313],
    desc: '松园宿舍区，西南侧生活组团。',
  },
  {
    id: 'huxi_canteen_01',
    name: '虎溪一食堂',
    category: 'canteen',
    campusId: 'd',
    coord: [106.298186, 29.597275],
    desc: '梅园/竹园生活区食堂，提供川渝风味与简餐。',
  },
  {
    id: 'huxi_canteen_02',
    name: '虎溪二食堂',
    category: 'canteen',
    campusId: 'd',
    coord: [106.295008, 29.596103],
    desc: '松园生活区食堂',
  },
  {
    id: 'huxi_canteen_03',
    name: '虎溪三食堂',
    category: 'canteen',
    campusId: 'd',
    coord: [106.299609, 29.590184],
    desc: '贴近学生活动中心与兰园食堂。',
  },
  {
    id: 'huxi_canteen_04',
    name: '虎溪四食堂',
    category: 'canteen',
    campusId: 'd',
    coord: [106.29838, 29.5982],
    desc: '清真食堂。',
  },
  {
    id: 'huxi_sports_01',
    name: '虎溪校区体育中心（大体育馆）',
    category: 'sports',
    campusId: 'd',
    coord: [106.305797, 29.597476],
    desc: '室内综合体育馆，篮球、羽毛球、体操等多功能场地。',
  },
  {
    id: 'huxi_sports_02',
    name: '虎溪校区足球场与田径场',
    category: 'sports',
    campusId: 'd',
    coord: [106.303627, 29.597057],
    desc: '标准400米田径场，含人造草坪足球场。',
  },
  {
    id: 'huxi_sports_03',
    name: '梅园室外篮球场与网球场',
    category: 'sports',
    campusId: 'd',
    coord: [106.301898, 29.597584],
    desc: '露天篮球场群与网球场，靠近梅园。',
  },
  {
    id: 'huxi_sports_04',
    name: '梅园小足球场与乒乓球场',
    category: 'sports',
    campusId: 'd',
    coord: [106.301876, 29.596584],
    desc: '露天小足球场与乒乓球场，靠近梅园。',
  },
  {
    id: 'huxi_sports_05',
    name: '兰园运动场',
    category: 'sports',
    campusId: 'd',
    coord: [106.297192, 29.588042],
    desc: '露天篮球场，羽毛球场与乒乓球场，靠近兰园。',
  },
  {
    id: 'huxi_sports_06',
    name: '游泳池',
    category: 'sports',
    campusId: 'd',
    coord: [106.292939, 29.594327],
    desc: '游泳考试时使用',
  },
  {
    id: 'huxi_sports_07',
    name: '松园运动场',
    category: 'sports',
    campusId: 'd',
    coord: [106.294525, 29.594896],
    desc: '露天篮球场，羽毛球场与乒乓球场，靠近松园',
  },
  {
    id: 'huxi_hospital_01',
    name: '虎溪校医院',
    category: 'hospital',
    campusId: 'd',
    coord: [106.300299, 29.598268],
    desc: '校内社区医院，提供基础诊疗与急救服务。',
  },
  {
    id: 'huxi_gate_01',
    name: '虎溪东门',
    category: 'gate',
    campusId: 'd',
    coord: [106.30649, 29.594166],
    desc: '面向大学城中路的主出入口。',
  },
  {
    id: 'huxi_gate_02',
    name: '虎溪西一门',
    category: 'gate',
    campusId: 'd',
    coord: [106.292736, 29.595718],
    desc: '西侧主出入口，邻近松园，有很多好吃的。',
  },
  {
    id: 'huxi_gate_03',
    name: '虎溪西二门',
    category: 'gate',
    campusId: 'd',
    coord: [106.291774, 29.592266],
    desc: '西侧次出入口，车辆可从这里进入。',
  },
  {
    id: 'huxi_gate_04',
    name: '虎溪西三门',
    category: 'gate',
    campusId: 'd',
    coord: [106.291994, 29.588458],
    desc: '不常用到的门，而且也经常不开',
  },
  {
    id: 'huxi_gate_05',
    name: '虎溪大北门',
    category: 'gate',
    campusId: 'd',
    coord: [106.301068, 29.598372],
    desc: '面向大学城南路的主出入口，去往熙街。',
  },
  {
    id: 'huxi_gate_06',
    name: '虎溪小北门',
    category: 'gate',
    campusId: 'd',
    coord: [106.298944, 29.598313],
    desc: '拿外卖点',
  },
  {
    id: 'huxi_gate_07',
    name: '虎溪北二门',
    category: 'gate',
    campusId: 'd',
    coord: [106.304138, 29.59841],
    desc: '这个门基本上不开',
  },
  {
    id: 'huxi_carstop_01',
    name: '虎溪校车站',
    category: 'busstation',
    campusId: 'd',
    coord: [106.303466, 29.595817],
    desc: '校车站，可从这里坐车',
  },
  {
    id: 'huxi_carstop_02',
    name: '理科楼乘车点',
    category: 'busstation',
    campusId: 'd',
    coord: [106.298148, 29.589145],
    desc: '可从这里坐车',
  },
  {
    id: 'huxi_carstop_03',
    name: '虎溪花园南乘车点',
    category: 'busstation',
    campusId: 'd',
    coord: [106.290784, 29.594313],
    desc: '可从这里坐车',
  },
  {
    id: 'huxi_landmark_01',
    name: '云湖',
    category: 'landmark',
    campusId: 'd',
    coord: [106.302619, 29.593744],
    desc: '靠近图书馆，有天鹅出没',
  },
  {
    id: 'huxi_landmark_02',
    name: '缙湖',
    category: 'landmark',
    campusId: 'd',
    coord: [106.299252, 29.593145],
    desc: '靠近图书馆，有天鹅出没',
  },
  {
    id: 'huxi_theater_01',
    name: '虎溪小剧场/学生活动中心',
    category: 'theater',
    campusId: 'd',
    coord: [106.299249, 29.5908],
    desc: '虎溪小剧场，也是学生活动中心所在地',
  },
  {
    id: 'huxi_theater_02',
    name: '虎溪大剧场',
    category: 'theater',
    campusId: 'd',
    coord: [106.29837, 29.590099],
    desc: '虎溪大剧场，学生活动与演出的地方',
  },
  {
    id: 'huxi_college_01',
    name: '电气学院楼',
    category: 'college',
    campusId: 'd',
    coord: [106.295771, 29.588802],
    desc: '电气学院楼',
  },
  {
    id: 'huxi_transit_01',
    name: '大学城轨道交通站',
    category: 'transit',
    campusId: 'd',
    coord: [106.308908, 29.60735],
    desc: '轨道交通1号线站点',
  },
  {
    id: 'huxi_transit_02',
    name: '尖顶坡轨道交通站',
    category: 'transit',
    campusId: 'd',
    coord: [106.292967, 29.607324],
    desc: '轨道交通1号线站点',
  },
  {
    id: 'huxi_express_01',
    name: '虎溪快递点',
    category: 'express',
    campusId: 'd',
    coord: [106.297572,29.598113],
    desc: '靠近一食堂',
  },
  {
    id: 'huxi_food_01',
    name: 'C3泰式食堂',
    category: 'food',
    campusId: 'd',
    coord: [106.293277,29.604003],
    desc: '虎溪这里还算比较好吃的泰餐。',
  },
  {
    id: 'huxi_food_02',
    name: '黄焖鸡·干辣椒抄手',
    category: 'food',
    campusId: 'd',
    coord: [106.292050,29.595458],
    desc: '推荐没有吃过的人真的要来吃一次，可以试试黄焖鸡里面加几颗抄手。',
  },
  {
    id: 'huxi_food_03',
    name: '西门重庆鸡公煲',
    category: 'food',
    campusId: 'd',
    coord: [106.292000,29.595503],
    desc: '性价比很高，可以喝免费汽水，味道也可以，很下饭。',
  },
  {
    id: 'huxi_food_04',
    name: '西门烧烤',
    category: 'food',
    campusId: 'd',
    coord: [106.292069,29.595134],
    desc: '西门这里的一家烧烤店，比较适合作为夜宵。',
  },
  {
    id: 'a_teaching_01',
    name: '主教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.470635, 29.565793],
    desc: '主教学楼，用于行政办公和上课。',
  },
  {
    id: 'a_teaching_02',
    name: '第一教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.468608, 29.568038],
    desc: '第一教学楼。',
  },
  {
    id: 'a_teaching_03',
    name: '第二教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.467702, 29.569261],
    desc: '第二教学楼。',
  },
  {
    id: 'a_teaching_04',
    name: '第三教学楼/资源与安全学院',
    category: 'teaching',
    campusId: 'a',
    coord: [106.467702, 29.569261],
    desc: '第三教学楼，现为资安学院楼。',
  },
  {
    id: 'a_teaching_05',
    name: '第四教学楼/离退休工作处',
    category: 'teaching',
    campusId: 'a',
    coord: [106.46549, 29.568574],
    desc: '第四教学楼，现为离退休工作处。',
  },
  {
    id: 'a_teaching_06',
    name: '第五教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.466938, 29.566302],
    desc: '第五教学楼，为主要教学楼之一。',
  },
  {
    id: 'a_teaching_07',
    name: '第六教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.466247, 29.564607],
    desc: '第六教学楼。',
  },
  {
    id: 'a_teaching_08',
    name: '第七教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.468115, 29.568509],
    desc: '第七教学楼，原机械与运载工程学院楼，现已搬迁至科学中心。',
  },
  {
    id: 'a_teaching_09',
    name: '第八教学楼',
    category: 'teaching',
    campusId: 'a',
    coord: [106.465483, 29.566393],
    desc: '第八教学楼，为主要教学楼之一。',
  },
  {
    id: 'a_dorm_01',
    name: '学生一宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.467148, 29.56402],
    desc: '学生一宿舍，在装修中。',
  },
  {
    id: 'a_dorm_02',
    name: '学生二宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.467793, 29.56352],
    desc: '学生二宿舍，在装修中。',
  },
  {
    id: 'a_dorm_03',
    name: '学生三宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.468535, 29.563045],
    desc: '学生三宿舍。',
  },
  {
    id: 'a_dorm_04',
    name: '学生四宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.469166, 29.562623],
    desc: '学生四宿舍。',
  },
  {
    id: 'a_dorm_05',
    name: '学生五宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.469151, 29.560881],
    desc: '学生五宿舍。',
  },
  {
    id: 'a_dorm_06',
    name: '学生六宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.46916, 29.56169],
    desc: '学生六宿舍。',
  },
  {
    id: 'a_dorm_07',
    name: '学生七宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.466721, 29.562545],
    desc: '学生七宿舍。',
  },
  {
    id: 'a_dorm_08',
    name: '学生八宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.46735, 29.56302],
    desc: '学生八宿舍。',
  },
  {
    id: 'a_dorm_09',
    name: '学生九宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.469151, 29.560881],
    desc: '学生九宿舍。',
  },
  {
    id: 'a_dorm_10',
    name: '学生十宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.466898, 29.561764],
    desc: '学生十宿舍。',
  },
  {
    id: 'a_dorm_11',
    name: '学生十一宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.467705, 29.56232],
    desc: '学生十一宿舍。',
  },
  {
    id: 'a_dorm_12',
    name: '学生十二宿舍',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.468227, 29.561993],
    desc: '学生十二宿舍。',
  },
  {
    id: 'a_dorm_13',
    name: '学生宿舍ACD楼',
    category: 'dormitory',
    campusId: 'a',
    coord: [106.467524, 29.561769],
    desc: '学生宿舍ACD楼。',
  },
  {
    id: 'a_canteen_01',
    name: '学生一食堂',
    category: 'canteen',
    campusId: 'a',
    coord: [106.466877, 29.563312],
    desc: '学生一食堂，一楼有麦当劳。',
  },
  {
    id: 'a_canteen_02',
    name: '民主湖食堂（学生四食堂）',
    category: 'canteen',
    campusId: 'a',
    coord: [106.46868, 29.562435],
    desc: '民主湖食堂。',
  },
  {
    id: 'a_canteen_03',
    name: '柏树林食堂（学生三食堂）',
    category: 'canteen',
    campusId: 'a',
    coord: [106.469893, 29.560915],
    desc: '柏树林食堂。',
  },
  {
    id: 'a_canteen_04',
    name: '东林教工食堂',
    category: 'canteen',
    campusId: 'a',
    coord: [106.464602, 29.56578],
    desc: '东林食堂。',
  },
  {
    id: 'a_canteen_05',
    name: '新华园食堂',
    category: 'canteen',
    campusId: 'a',
    coord: [106.467374, 29.56991],
    desc: '新华园食堂。',
  },
  {
    id: 'a_library_01',
    name: 'A区图书馆',
    category: 'library',
    campusId: 'a',
    coord: [106.46834, 29.565402],
    desc: 'A区图书馆，设有多层自习区与研讨室。',
  },
  {
    id: 'a_sports_01',
    name: '思群广场',
    category: 'sports',
    campusId: 'a',
    coord: [106.468322, 29.564328],
    desc: '室外操场，足球场。',
  },
  {
    id: 'a_sports_02',
    name: '团结广场',
    category: 'sports',
    campusId: 'a',
    coord: [106.469169, 29.566284],
    desc: '室外操场，足球场。',
  },
  {
    id: 'a_sports_03',
    name: '篮球场（靠近钟塔）',
    category: 'sports',
    campusId: 'a',
    coord: [106.468128, 29.567122],
    desc: '靠近钟塔的篮球场。',
  },
  {
    id: 'a_sports_04',
    name: '风雨操场',
    category: 'sports',
    campusId: 'a',
    coord: [106.471981, 29.564492],
    desc: '靠近体育馆。',
  },
  {
    id: 'a_sports_05',
    name: '体育馆',
    category: 'sports',
    campusId: 'a',
    coord: [106.472226, 29.564279],
    desc: '可在重大后勤-场馆预约中预约羽毛球，乒乓球场地。',
  },
  {
    id: 'a_sports_06',
    name: '篮球场/羽毛球场/网球场（靠近图书馆）',
    category: 'sports',
    campusId: 'a',
    coord: [106.466915, 29.56568],
    desc: '靠近图书馆的篮球场，羽毛球场和网球场',
  },
  {
    id: 'a_sports_07',
    name: '乒乓球场',
    category: 'sports',
    campusId: 'a',
    coord: [106.469415, 29.565071],
    desc: '室外乒乓球场，点位为近似位置',
  },
  {
    id: 'a_admin_01',
    name: '办公楼',
    category: 'admin',
    campusId: 'a',
    coord: [106.468748, 29.565564],
    desc: 'A区办公楼，也为行政处',
  },
  {
    id: 'a_admin_02',
    name: '办公楼',
    category: 'admin',
    campusId: 'a',
    coord: [106.469015, 29.565397],
    desc: 'A区财务处，投递报销单的地方',
  },
  {
    id: 'a_gate_01',
    name: 'A区中门',
    category: 'gate',
    campusId: 'a',
    coord: [106.467012, 29.561711],
    desc: '靠近十舍；设门禁，午夜后关闭',
  },
  {
    id: 'a_gate_02',
    name: 'A区南一门',
    category: 'gate',
    campusId: 'a',
    coord: [106.470113, 29.55999],
    desc: '面向沙中路的出入口，这个门没有门禁，也可出入车辆。',
  },
  {
    id: 'a_gate_03',
    name: 'A区东一门（柏树林门）',
    category: 'gate',
    campusId: 'a',
    coord: [106.472131, 29.561537],
    desc: '面向松林路的出入口。',
  },
  {
    id: 'a_gate_04',
    name: 'A区东门（校医院门）',
    category: 'gate',
    campusId: 'a',
    coord: [106.472428, 29.562889],
    desc: '校医院旁边的门，进出校医院可走这里。',
  },
  {
    id: 'a_gate_05',
    name: 'A区东南门（体育学院门）',
    category: 'gate',
    campusId: 'a',
    coord: [106.472673, 29.563667],
    desc: '靠近体育学院。',
  },
  {
    id: 'a_gate_06',
    name: '江边电梯',
    category: 'gate',
    campusId: 'a',
    coord: [106.471948, 29.565361],
    desc: '可从这里进出校园，下电梯直达江边。',
  },
  {
    id: 'a_gate_07',
    name: 'A区西北门',
    category: 'gate',
    campusId: 'a',
    coord: [106.465015, 29.570122],
    desc: '西北门。',
  },
  {
    id: 'a_gate_08',
    name: 'A区正大门（正门）',
    category: 'gate',
    campusId: 'a',
    coord: [106.464181, 29.567418],
    desc: 'A校园正门',
  },
  {
    id: 'a_gate_09',
    name: 'A区后门（松林坡门）',
    category: 'gate',
    campusId: 'a',
    coord: [106.472394,29.563486],
    desc: 'A校园松林坡门，往这里走下去有很多好吃的。',
  },
  {
    id: 'a_hospital_01',
    name: '校医院',
    category: 'hospital',
    campusId: 'a',
    coord: [106.472216, 29.563117],
    desc: '',
  },
  {
    id: 'a_busstation_01',
    name: '后门校车站',
    category: 'busstation',
    campusId: 'a',
    coord: [106.471154, 29.562414],
    desc: '',
  },
  {
    id: 'a_busstation_02',
    name: '钟塔校车站',
    category: 'busstation',
    campusId: 'a',
    coord: [106.467243, 29.567197],
    desc: '',
  },
  {
    id: 'a_landmark_01',
    name: '钟塔',
    category: 'landmark',
    campusId: 'a',
    coord: [106.467473, 29.567312],
    desc: '',
  },
  {
    id: 'a_landmark_02',
    name: '防空洞爱国教育基地',
    category: 'landmark',
    campusId: 'a',
    coord: [106.46731, 29.56792],
    desc: '',
  },
  {
    id: 'a_food_01',
    name: '中渡芋圆',
    category: 'food',
    campusId: 'a',
    coord: [106.472875, 29.564787],
    desc: 'A校园附近甜品店',
  },
  {
    id: 'a_food_02',
    name: '曾一刀私房牛肉面',
    category: 'food',
    campusId: 'a',
    coord: [106.462373, 29.558773],
    desc: '这是重庆一三八高中生下课吃面的常驻地，笔者最推荐其原汤牛肉牛筋混合韭叶面加蛋，其中“原汤”并不指浅白色的牛骨汤，而是指浮于汤面上的那层微辣牛油，这是整碗面的精髓；单吃牛肉太柴，单吃牛筋太腻，牛肉牛筋混合这一搭配折中两者优缺，让吃面也不再单调；韭叶面由其形状酷似韭菜叶子得名，源于四川达州大竹县，其优点是在吃面时可裹上更多红油，牛油香味更浓；在吃面时煎蛋埋于碗底，面吃到一半时，煎蛋已吸饱汤汁，此时再吃味更美(๑´ڡ`๑)（from 巧克力大王）',
  },
 {
    id: 'a_food_03',
    name: '吹得神豌杂面(小龙坎新街店)',
    category: 'food',
    campusId: 'a',
    coord: [106.464133,29.558334],
    desc: '这家店以干馏豌杂见长，笔者推荐干馏豌杂加蛋，是否加辣取决于个人，加不加辣都好吃。这家店的干馏豌杂面面条较粗，豌杂酱汁收得咸香浓稠，调料沉在碗底，因此吃前需要把面与调料拌匀（用重庆话说就是“霍转”），不然吃到最后会过咸，煎蛋需事先埋于碗底，这样最后吃到煎蛋时，煎蛋早已吸饱酱汁，吃起来更有味。（from 巧克力大王）'
  },
  {
    id: 'a_food_04',
    name: '毛烧烤(工人村店)',
    category: 'food',
    campusId: 'a',
    coord: [106.468068,29.557186],
    desc: '这是一家极具特色的烧烤店，点单方式类似麻辣烫，自行选菜然后让后厨加工。笔者推荐小芋头和蜜汁烤翅，这家店选用的芋头是重庆老芋仔，其历史最早可追溯至清朝晚期，个头小，口感顺滑，带有一丝奶香。烤制过后外层轻微失水，质地似镜面蛋糕的淋面，用嘴抿开表面，绵密微烫的芋头便裹挟着干辣椒与香葱化开在舌尖，这样的烤芋头在别处很难吃到。蜜汁烤翅也是一绝，后厨在烤制时会反复刷酱，鸡翅吃起来又香又甜又嫩，一个人至少要吃两串（四个鸡翅）才能吃得尽兴。吃这家烧烤时推荐配店里的大窑或街对面的蜜雪冰城。（from 巧克力大王）'
  },
  {
    id: 'a_food_05',
    name: 'Clean_Cup自烘焙咖啡店',
    category: 'food',
    campusId: 'a',
    coord: [106.463645,29.563953],
    desc: '位于欣阳广场旁小巷的深处，是一家自主烘焙咖啡豆的门店，店内装修风格温暖，有可爱的小猫。笔者推荐这家店的美式咖啡，这家店豆单一直在更新，豆子可凭自己喜好选择，在这家店喝咖啡，你能清晰地感受到精品咖啡的魅力，不同温度下咖啡的风味是清晰且不同的。（from 巧克力大王）'
  },
  {
    id: 'a_food_06',
    name: '沈姐烤鱼',
    category: 'food',
    campusId: 'a',
    coord: [106.463131,29.570069],
    desc: '相信坐过环线的同学们都听过沈姐烤鱼的广告，老校区这边的沈姐烤鱼位于重庆七中旁的巷子里。这家店的烤鱼好吃，干锅也不赖，笔者喜欢吃这家店的肥肠干锅，肥肠处理得很干净，切成小段，炒得肠皮微焦香气十足。（from 巧克力大王）',
  },
  {
    id: 'a_food_07',
    name: 'guoguo䴹果',
    category: 'food',
    campusId: 'a',
    coord: [106.463535,29.564095],
    desc: '就在cleancup coffee旁边，该店的抹茶制品做得十分出色，笔者推荐抹茶麻薯米面包，内馅是冰冰凉凉的抹茶奶油，外面裹着糯糯的鲜奶麻薯，最外层是很本分的，让人安心的米面包。推荐在这家面包店买点面包，然后到隔壁咖啡店坐着点杯咖啡消磨时光。（from 巧克力大王）',
  },
  {
    id: 'a_food_08',
    name: '大粽小吃',
    category: 'food',
    campusId: 'a',
    coord: [106.463533,29.565155],
    desc: '是一家开在沙坪坝小学旁的老店，中国自古以来便有甜咸粽之争，但在这家店，双方可以和平共处，因为这家店有卖咸粽甜粽甚至白粽。作为咸粽党，笔者推荐咸蛋黄肉粽，个头大，粽子里有整颗咸蛋黄，肉是带有一定油脂的，油脂化开在糯米上，吃着很香。（from 巧克力大王）',
  },
  {
    id: 'a_food_09',
    name: '不养鱼咖啡',
    category: 'food',
    campusId: 'a',
    coord: [106.459993,29.558256],
    desc: '这家店对初来的同学有点难找，位于南开步行街卤校长火锅背后的巷子里。这是一家一人经营的独立咖啡门店，老板是多年咖啡师，会详细向顾客介绍咖啡出品，咖啡价格较为合理，笔者推荐拿铁，这家店的特调比较出名，笔者还没喝过，或许值得一试。（from 巧克力大王）',
  },
  {
    id: 'a_transit_01',
    name: '重庆大学轨道交通站',
    category: 'transit',
    campusId: 'a',
    coord: [106.463616, 29.568332],
    desc: '轨道交通环线站点',
  },
  {
    id: 'a_express_01',
    name: 'A区邮政快递点',
    category: 'express',
    campusId: 'a',
    coord: [106.467409,29.561742],
    desc: '邮政快递可以在这里拿取。',
  },
  {
    id: 'a_express_02',
    name: 'A区主快递点',
    category: 'express',
    campusId: 'a',
    coord: [106.468178,29.562029],
    desc: '顺丰/京东/圆通/申通/拼多多快递都可在这里拿取。',
  },
  {
    id: 'b_teaching_01',
    name: '第二综合楼',
    category: 'teaching',
    campusId: 'b',
    coord: [106.461484, 29.566761],
    desc: 'B校园主要公共教学楼',
  },
  {
    id: 'b_teaching_02',
    name: '第一综合楼',
    category: 'teaching',
    campusId: 'b',
    coord: [106.459751, 29.566117],
    desc: '教学与办公楼',
  },
  {
    id: 'b_teaching_03',
    name: '建工馆',
    category: 'teaching',
    campusId: 'b',
    coord: [106.461012, 29.567465],
    desc: 'B校园标志性教学建筑',
  },
  {
    id: 'b_library_01',
    name: 'B区图书馆',
    category: 'library',
    campusId: 'b',
    coord: [106.460531, 29.566551],
    desc: 'B校园图书馆',
  },
  {
    id: 'b_college_01',
    name: '建筑城规学院',
    category: 'college',
    campusId: 'b',
    coord: [106.461622, 29.568165],
    desc: '建筑城规学院院楼',
  },
  {
    id: 'b_college_02',
    name: '管理科学与房地产学院',
    category: 'college',
    campusId: 'b',
    coord: [106.460562, 29.565734],
    desc: '管理科学与房地产学院院楼',
  },
  {
    id: 'b_college_03',
    name: '材料楼',
    category: 'college',
    campusId: 'b',
    coord: [106.462484, 29.567367],
    desc: '材料相关教学科研楼',
  },
  {
    id: 'b_hospital_01',
    name: 'B区校医院',
    category: 'hospital',
    campusId: 'b',
    coord: [106.462545, 29.567922],
    desc: 'B校园医疗服务点',
  },
  {
    id: 'b_sports_01',
    name: 'B区足球场',
    category: 'sports',
    campusId: 'b',
    coord: [106.459923, 29.567912],
    desc: '田径与足球场',
  },
  {
    id: 'b_canteen_01',
    name: '学生二食堂',
    category: 'canteen',
    campusId: 'b',
    coord: [106.458829, 29.564575],
    desc: 'B校园主要食堂',
  },
  {
    id: 'b_dorm_01',
    name: '学生三舍',
    category: 'dormitory',
    campusId: 'b',
    coord: [106.457658, 29.564653],
    desc: '学生宿舍',
  },
  {
    id: 'b_dorm_02',
    name: '学生四舍',
    category: 'dormitory',
    campusId: 'b',
    coord: [106.456967, 29.564928],
    desc: '学生宿舍',
  },
  {
    id: 'b_transit_01',
    name: '重庆大学轨道交通站',
    category: 'transit',
    campusId: 'b',
    coord: [106.463616, 29.568332],
    desc: '轨道交通环线站点',
  },
  {
    id: 'b_food_01',
    name: '黑娃餐馆(重大B区店)',
    category: 'food',
    campusId: 'b',
    coord: [106.459129,29.563863],
    desc: '好吃，味道足'
  },
  {
    id: 'b_food_02',
    name: '诗熠烤米线',
    category: 'food',
    campusId: 'b',
    coord: [106.463060,29.568446],
    desc: '这家店在重大B区大门对面，或许是重庆唯一一家烤米线，做法比较有特色，是用锡纸包裹米线及其他食材进行烤制，口味偏咸香，卫生条件堪忧但味道不错。（from 巧克力大王）',
  },
  {
    id: 'c_teaching_01',
    name: '第一教学楼',
    category: 'teaching',
    campusId: 'c',
    coord: [106.455036, 29.559969],
    desc: 'C校园公共教学楼',
  },
  {
    id: 'c_transit_01',
    name: '重庆大学轨道交通站',
    category: 'transit',
    campusId: 'c',
    coord: [106.463616, 29.568332],
    desc: '轨道交通环线站点',
  },
  {
    id: 'c_carstop_01',
    name: 'C区乘车点',
    category: 'busstation',
    campusId: 'c',
    coord: [106.455649,29.560364],
    desc: '可从这里坐车',
  },
  {
    id: 'e_teaching_01',
    name: '教学楼A栋',
    category: 'teaching',
    campusId: 'e',
    coord: [106.811052, 29.739938],
    desc: '卓越工程师学院教学楼',
  },
  {
    id: 'e_teaching_02',
    name: '教学楼B栋',
    category: 'teaching',
    campusId: 'e',
    coord: [106.810559, 29.739415],
    desc: '卓越工程师学院教学楼',
  },
  {
    id: 'e_teaching_03',
    name: '教学楼C栋',
    category: 'teaching',
    campusId: 'e',
    coord: [106.810276, 29.739832],
    desc: '卓越工程师学院教学楼',
  },
  {
    id: 'e_dorm_01',
    name: '学生宿舍1栋',
    category: 'dormitory',
    campusId: 'e',
    coord: [106.807427, 29.739101],
    desc: '学生宿舍',
  },
  {
    id: 'e_dorm_02',
    name: '学生宿舍2栋',
    category: 'dormitory',
    campusId: 'e',
    coord: [106.807657, 29.738741],
    desc: '学生宿舍',
  },
  {
    id: 'e_canteen_01',
    name: '两江校区食堂',
    category: 'canteen',
    campusId: 'e',
    coord: [106.808541, 29.7406],
    desc: '校园食堂',
  },
  {
    id: 'e_sports_01',
    name: '两江校区运动场',
    category: 'sports',
    campusId: 'e',
    coord: [106.809396, 29.738322],
    desc: '田径与足球场',
  },
  {
    id: 'e_busstation_01',
    name: '桐梓林轨道交通站',
    category: 'transit',
    campusId: 'e',
    coord: [106.815265, 29.741924],
    desc: '轨道交通4号线站点',
  },
] as const satisfies readonly Building[];
