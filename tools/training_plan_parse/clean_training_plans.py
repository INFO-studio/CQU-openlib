...
COURSE_ANNOTATION_RE = re.compile(
    r"(?:英|英文|全英|全英文|英语授课|英文授课|全英文授课|双语|双语课|双语教学|双语授课|"
    r"研讨课|研讨|本研共享|本硕共享|本博共享|本科生研究生共享|拔尖|卓越|教学|分散|通用|"
    r"创新实践课|含.+|包括.+|[A-Z]|[IVX]+|\d+(?:st|nd|rd|th)?|[一二三四五六七八九十上下中]+|"
    r"(?:[IVX]+|\d+)[-－—](?:[IVX]+|\d+))"
)
...
def simplify_course_name(name: str) -> str:
    ...
    def annotation(match: re.Match[str]) -> str:
        tag = unicodedata.normalize("NFKC", match[1]).strip()
        if not tag or COURSE_ANNOTATION_RE.fullmatch(re.sub(r"\s+", "", tag)):
            return ""
        return match[0]
    ...
    while previous != name:
        previous = name
        name = re.sub(r"[（(]([^（）()]*)[）)]", annotation, name).strip()
        name = re.sub(r"\s*[-－—]?\s*\d+(?:st|nd|rd|th)?$", "", name).strip()
        name = re.sub(r"\s*[-－—]?\s*[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫＩＶＸ]+$", "", name).strip()
        # ASCII 罗马数字只接受词边界，不能截断 CAD、UML、CAM 等核心缩写。
        name = re.sub(r"(?<=[\u3400-\u9fff\s）)])[-－—]?\s*[IVX]+$", "", name).strip()
    name = re.sub(r"^留学生[\s-]*", "", name)
    name = re.sub(r"^文明经典(?:系列)?[AB]?$", "文明经典系列", name)
    if "体育" in name and name != "体育心理学":
        return "体育"
    return re.sub(r"fourier", "Fourier", name, flags=re.I)
...
class CourseLinks:
    ...
    def resolve(self, code: str | None, name: str | None) -> tuple[str | None, str]:
        if name:
            canonical = "/course/" + simplify_course_name(name)
            if canonical in self.pages:
                return canonical, "name" if canonical == "/course/" + name else "simplified_name"
