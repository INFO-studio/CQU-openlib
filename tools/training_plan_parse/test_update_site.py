...
class UpdateTests(unittest.TestCase):
    def test_sports_links_merge_without_changing_display_names(self):
        links = plans.CourseLinks(["/course/体育", "/course/大学体育核心素质课", "/course/体育心理学"], {})
        for name in ("大学体育核心素质课", "大学体育核心素质课（1）（英）", "体育自选项目1", "体育自选项目2", "体育自选项目3", "体育基础（1）", "体育概论", "学校体育学", "体育专业英语（1）"):
            with self.subTest(name=name):
                ...
                self.assertIn(f"[{name}](../../../course/体育.md)", plans.render_course(record))
    ...
    def test_placeholder_uses_core_name_for_all_variants(self):
        data = clean(row(课程名称="新课程I", 课程代码="N1"), row(课程名称="新课程II（英）", 课程代码="N2"))
        update.complete_links(data["records"], plans.CourseLinks([], {}))
        self.assertEqual({r["course_page"] for r in data["records"]}, {"/course/新课程"})
        self.assertEqual({r["code"] for r in data["records"]}, {"N1", "N2"})
        self.assertEqual({r["name"] for r in data["records"]}, {"新课程I", "新课程II（英）"})

    def test_conflicting_code_uses_exact_name_without_picking_winner(self):
        links = plans.CourseLinks(["/course/甲", "/course/乙"], {"courses": {
            "/course/甲": {"codes": ["X"]}, "/course/乙": {"codes": ["X"]},
        }})
    ...
    def test_dirty_metadata_does_not_recreate_dirty_pages(self):
        links = plans.CourseLinks(["/course/新课程（英）"], {"courses": {
    ...
    def test_writes_are_idempotent_preserve_resources_and_fill_broken_links(self):
        with TemporaryDirectory() as directory:
            ...
            old_content = "* [遗留（1）（英）](../../../course/遗留.md) - :l-book:`OLD1`\n"
            old_plan.write_text(old_content, encoding="utf-8")
            links = plans.CourseLinks(["/course/已有"], {})
            data = clean(row(课程名称="已有"), row(课程名称="新课", 课程代码="NEW"), links=links)
            bundle: plans.Bundle = {**data, "markdown_previews": plans.render_previews(data["records"])}
            ...
            self.assertEqual((root / "course/新课.md").read_text(encoding="utf-8"), update.PLACEHOLDER)
    ...
    def test_migration_normalizes_all_placeholders_and_preserves_actual_content(self):
        with TemporaryDirectory() as directory:
            ...
            old.write_text(update.with_updated('## 资源  \n=== ":l-book:`N1`"  \n    暂无资料，欢迎贡献。  \n\n' + update.PLACEHOLDER, "", "2026-09-15"), encoding="utf-8")
            canonical = courses / "新课.md"
            canonical.write_text("## 资源\n实际教材\n", encoding="utf-8")
            empty = courses / "空课.md"
            empty.write_text("", encoding="utf-8")
            old_style = courses / "旧占位.md"
            old_style.write_text(update.with_updated(update.PLACEHOLDER, "", "2020-01-01"), encoding="utf-8")
            partial = courses / "另一门（2）.md"
            partial.write_text(update.PLACEHOLDER + "\n已知教材：《书名》\n", encoding="utf-8")
            old_plan = root / "academic/专业培养方案/旧学院/旧专业.md"
            old_plan.parent.mkdir(parents=True)
            old_plan.write_text("* [新课（1）（英）](../../../course/新课（1）（英）.md) - :l-book:`N1`\n", encoding="utf-8")
            article = root / "引用.md"
            article.write_text("[新课](course/新课（1）（英）.md)\n[旧课](../course/新课%EF%BC%881%EF%BC%89%EF%BC%88英%EF%BC%89.md#资源)\n", encoding="utf-8")
            ...
            self.assertEqual(canonical.read_text(encoding="utf-8"), "## 资源\n实际教材\n")
            self.assertIn("已知教材", partial.read_text(encoding="utf-8"))
            self.assertEqual(empty.read_text(encoding="utf-8"), update.PLACEHOLDER)
            self.assertEqual(old_style.read_text(encoding="utf-8"), update.PLACEHOLDER)
            self.assertIn("[新课（1）（英）](../../../course/新课.md)", old_plan.read_text(encoding="utf-8"))
            ...
            self.assertEqual(update.write_updates(update.prepare_updates(bundle, root, links), "2026-09-16"), 0)
    ...
    def test_nested_placeholders_keep_their_directory_and_links(self):
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                update.rewrite_course_links("[德语](../../../course/非限课/基础德语（1）.md)", renames),
                "[德语](../../../course/非限课/基础德语.md)",
            )
            data = clean(row())
            bundle: plans.Bundle = {**data, "markdown_previews": plans.render_previews(data["records"])}
            update.write_updates(update.prepare_updates(bundle, root, plans.CourseLinks([], {})), "2026-09-15")
