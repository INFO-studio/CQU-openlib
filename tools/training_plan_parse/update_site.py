...
def is_placeholder(content: str) -> bool:
    return not existing_body(content).strip()


def with_placeholder(previous: str) -> str:
    match = frontmatter_re.match(previous)
    metadata = match[1] if match else ""
    if re.search(r"(?m)^placeholder:.*$", metadata):
        metadata = re.sub(r"(?m)^placeholder:.*$", "placeholder: course", metadata)
    else:
        metadata = f"{metadata}\nplaceholder: course".lstrip("\n")
    return f"---\n{metadata}\n---\n"
...
def with_updated(body: str, previous: str, updated: str) -> str:
    match = frontmatter_re.match(previous)
    metadata = match[1] if match else ""
    metadata = re.sub(r"(?m)^placeholder:.*\n?", "", metadata).strip()
...
def rewrite_course_links(content: str, renames: dict[Path, Path]) -> str:
    names = {
        old.as_posix().split("/course/", 1)[1].removesuffix(".md"): new.as_posix()
        .split("/course/", 1)[1]
        .removesuffix(".md")
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if name not in names:
            return match[0]
        target = "".join(
            quote(char, safe="/.-_~") if ord(char) < 128 else char
...
def prepare_updates(
    bundle: plans.Bundle, doc_root: Path, links: plans.CourseLinks
) -> dict[Path, str]:
    ...
    for path in sorted(doc_root.rglob("*.md")):
        ...
        if path.is_relative_to(doc_root / "course") and is_placeholder(previous):
            if previous != with_placeholder(previous):
                updates[path] = placeholder
        elif body != existing_body(previous):
            updates[path] = body
...
def write_updates(updates: dict[Path, str], updated: str) -> int:
    ...
    for path, body in sorted(updates.items()):
        ...
        if body == placeholder:
            content = with_placeholder(previous)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="预览或写入培养方案、统一课程占位页，并刷新课程号元数据。"
    )
    parser.add_argument(
        "--write", action="store_true", help="显式写入站点；默认只输出数量摘要"
    )
    parser.add_argument(
        "--prune-placeholders",
        action="store_true",
        help="删除已迁移到核心名称的旧纯占位页，需同时指定 --write",
    )
    parser.add_argument(
        "--date",
        default=datetime.now(tz=ZoneInfo("Asia/Shanghai")).date().isoformat(),
        help="内容更新日期 YYYY-MM-DD",
    )
    ...
    if (
        args.expect_rows is not None
        and bundle["summary"]["input_rows"] != args.expect_rows
    ):
    ...
    summary = {
        **bundle["summary"],
        "course_placeholders": sum(
            path.is_relative_to(doc_root / "course") for path in updates
        ),
